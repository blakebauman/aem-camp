...
**On first use:**
1. Skills auto-activate based on your prompts
2. Hooks enforce best practices (like content-first development)
3. Commands available via `/command-name`
4. Persistent docs loaded automatically on session start

**Common workflows:**
- Create block: `/new-block hero` or "Create a hero block"
- Test changes: `/test-block hero` or "Test my hero block"
- Find content: `/find-content hero` or "Find pages with hero block"
- Search docs: `/search-docs fragments` or "How do fragments work in AEM?"
...
Skills automatically activate based on your prompts thanks to the `user-prompt-submit.js` hook:

- **"create a block"** → auto-activates `content-driven-development`
- **"test changes"** → auto-activates `testing-blocks`  
- **"content model"** → auto-activates `content-modeling`
- **"find example"** → auto-activates `block-collection-and-party`
- **"how to [AEM feature]"** → auto-activates `docs-search`

You can also invoke skills explicitly by mentioning them or using commands.
...
Complex tasks can be delegated to specialized agents in `.claude/agents/`. Agents have focused expertise and can work autonomously on multi-step tasks.
...
Agents activate automatically based on task complexity, or delegate explicitly:

```
@block-developer Create a complete testimonial carousel block
@testing-specialist Validate my changes are ready for PR
@docs-expert Find examples of lazy loading patterns
```
...
Hooks automatically trigger at specific workflow points to enforce best practices and provide guidance.

### Active Hooks

**`user-prompt-submit.js`** - Analyzes prompts and auto-activates skills
- Runs before Claude processes your request
- Pattern matches to activate relevant skills
- Suggests agents for complex tasks

**`pre-tool-use.js`** - Validates before tool execution
- Enforces content-first development (blocks code changes without test content)
- Checks prerequisites and dependencies
- Provides guidance when violations detected

**`post-tool-use.js`** - Follows up after tool execution
- Auto-runs linting after code changes
- Suggests next steps in workflow
- Tracks progress and recommends commits

**`session-start.js`** - Initializes new sessions
...
Slash commands provide quick access to common workflows. Type `/` to see available commands.
...
- `/find-examples <query>` - Find reference block implementations

**See:** `.claude/commands/README.md` for detailed command documentation.

## Persistent Documentation

Critical project knowledge is stored in `.claude/docs/` and loaded automatically on session start. This ensures knowledge survives context resets.
...
- No need to re-explain architecture after context resets
- Single source of truth for patterns and decisions

**See:** `.claude/docs/README.md` for documentation system details.

## Examples

Comprehensive blog domain examples demonstrate all patterns in `.claude/examples/blog/`:

- **Example blocks** with full implementations
- **Test content** showing content models
- **Documentation** of patterns and practices
- **Progressive learning** from simple to advanced

**Featured example:** `blog-post-card` - Complete block showing:
- Content-driven development approach
- Author-friendly content model
- Clean decoration patterns
- Responsive CSS with variants
- Comprehensive comments

**See:** `.claude/examples/blog/README.md` for all examples.
